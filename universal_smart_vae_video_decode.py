...
class SmartVAE_StreamingDecoder:
    ...
    def _normalize_output(self, tensor, aspect_ratio: Optional[float] = None) -> torch.Tensor:
        ...
        detection. Original sorted([F, H, W]) by numeric size to find frames —
        breaks when F > H or F > W (e.g. 544×544 with 70+ latent frames),
        causing ghost/striped frames. New code identifies C by value (3 or 4),
        then derives F, H, W from fixed positional rules.
        """
        if isinstance(tensor, (list, tuple)):
            if not tensor:
                raise ValueError("VAE returned empty output")
